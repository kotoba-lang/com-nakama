(ns nakama.main
  "Kotodama WASM entrypoint for the Nakama clean-room actor (L4) — Clojure port.
  ...
  facts (`nakama.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nakama")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nakama.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nakama-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nakama-compat/ready)
